...
const ImageCard = ({ image }) => {
    ...
    return (
        <div className='max-w-sm rounded overflow-hidden shadow-lg hover:scale-105 duration-500'>
            ...
            <div className="px-6 py-4">
                {tags.map((tag, index) => (
                    <span key={index} className="inline-block bg-gray-200 rounded-full px-3 text-sm font-semibold text-gray-700 mr-2">#{tag}</span>
                ))}
            </div>
        </div>
    )
